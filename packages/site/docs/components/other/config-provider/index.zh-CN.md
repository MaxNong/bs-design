...
```jsx | pure
import { ConfigProvider } from 'bs-design';
...
<code src="demos/other/config-provider/locale.tsx" title="国际化" desc="此处列出 Ant Design 中需要国际化支持的组件，你可以在演示里切换语言。"></code>

<code src="demos/other/config-provider/direction.tsx" title="方向" desc="这里列出了支持 `rtl` 方向的组件，您可以在演示中切换方向。"></code>

<code src="demos/other/config-provider/size.tsx" title="组件尺寸" desc="修改默认组件尺寸。"></code>

<code src="demos/other/config-provider/theme.tsx" title="全局样式" desc="通过 css variable 修改全局主题色（你可以切换到组件页面查看更详细的样式展示），不支持 IE。自动生成的变量可能会根据设计调整，请勿直接依赖。详细配置请[点击查看](/docs/react/customize-theme-variable)。"></code>

<code src="demos/other/config-provider/prefixCls.tsx" title="前缀" desc="修改组件和图标前缀。"></code>
